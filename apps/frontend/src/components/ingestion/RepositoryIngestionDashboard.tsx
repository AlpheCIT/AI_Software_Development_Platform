    ...
    const loadPastRuns = async () => {
      ...
      try {
        ...
        const pastJobs = data.runs.slice(0, 10).map((run: any) => ({
          ...
          status: run.status === 'running' ? 'running' as const
                : run.status === 'completed' ? 'completed' as const : 'failed' as const,
          progress: run.status === 'completed' ? 100 : run.status === 'running' ? 50 : 0,
          phase: run.status === 'running' ? 'Running' : 'Completed',
          startTime: new Date(run.startedAt || run.date),
          endTime: run.completedAt ? new Date(run.completedAt) : undefined,
          collectionsPopulated: run.executionLog?.length || 0,
          totalCollections: run.selectedAgents?.length || 22,
          metrics: {
            filesProcessed: run.filesAnalyzed || run.testsGenerated || 0,
            nodesCreated: run.executionLog?.filter((e: any) => e.status === 'success').length || 0,
            securityIssues: 0,
            performanceIssues: 0,
          },
        }));
